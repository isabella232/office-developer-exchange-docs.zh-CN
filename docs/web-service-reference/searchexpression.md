...
description: SearchExpression 元素是一个抽象元素，表示限制中的替换元素。 所有搜索表达式都从此基类型派生。 此元素不用于 XML 实例文档。
...
**SearchExpression** 元素是一个抽象元素，表示限制中的替换元素。 所有搜索表达式都从此基类型派生。 此元素不用于 XML 实例文档。 
...
任何属于 SearchExpression 替换组的 filter 元素都可以代替 SearchExpression 元素显示。
