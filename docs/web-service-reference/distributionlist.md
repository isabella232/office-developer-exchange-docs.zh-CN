...
|**元素**|**说明**|
|:-----|:-----|
|[MimeContent](mimecontent.md) <br/> |包含 base64Binary 格式表示的对象的本机 MIME 流。  <br/> |
|[ItemId](itemid.md) <br/> |包含在 Exchange 存储中的通讯组列表项的唯一标识符和更改键。  <br/> |
|[ParentFolderId](parentfolderid.md) <br/> |表示包含通讯组列表项的父文件夹的标识符。  <br/> |
|[ItemClass](itemclass.md) <br/> |表示通讯组列表项目的消息类。  <br/> |
|[Subject](subject.md) <br/> |代表 Exchange 存储项和响应对象主题。  <br/> |
|[Sensitivity](sensitivity.md) <br/> |包含通讯组列表项的敏感度的状态。  <br/> |
|[Body](body.md) <br/> |表示通讯组列表项目的实际正文内容。  <br/> |
|[附件](attachments-ex15websvcsotherref.md) <br/> |包含的项或附加到 Exchange 存储中的通讯组列表项的文件。  <br/> |
|[DateTimeReceived](datetimereceived.md) <br/> |表示的日期和已接收邮箱中的通讯组列表项的时间。  <br/> |
|[Size](size.md) <br/> |表示的大小，以字节为单位的通讯组列表项目。 此属性是只读的。  <br/> |
|[类别](categories-ex15websvcsotherref.md) <br/> |表示标识的邮箱中通讯组列表项目所属的类别的字符串的集合。  <br/> |
|[Importance](importance.md) <br/> |介绍通讯组列表项的重要性。  <br/> |
|[InReplyTo](inreplyto.md) <br/> |表示此项是答复 （英文） 中的项目的标识符。  <br/> |
...
|[ExtendedProperty](extendedproperty.md) <br/> |标识通讯组列表项上的扩展的属性。  <br/> |
|[区域性](culture.md) <br/> |表示邮箱中的通讯组列表项的区域性。  <br/> |
...
|[FileAs](fileas.md) <br/> |表示通讯组列表联系人文件夹中的字段如何。  <br/> |
|[ContactSource](contactsource.md) <br/> |描述联系人是否位于 Exchange 存储中或 Active Directory 域服务 (AD DS) 中。  <br/> |
...
|[AppendToItemField](appendtoitemfield.md) <br/> |标识要追加到通讯组列表的单个属性[UpdateItem 操作](updateitem-operation.md)期间数据。  <br/> |
|[ConflictingMeetings](conflictingmeetings.md) <br/> |标识与会议时间冲突的所有项目。  <br/> |
|[创建 (ItemSync)](create-itemsync.md) <br/> |标识本地客户端存储中创建的单个通讯组列表。  <br/> |
|[更新 (ItemSync)](update-itemsync.md) <br/> |标识更新本地客户端存储中的单个通讯组列表。  <br/> |
...
|[SetItemField](setitemfield.md) <br/> |向通讯组列表项[UpdateItem 操作](updateitem-operation.md)中的单个属性表示更新。  <br/> |
