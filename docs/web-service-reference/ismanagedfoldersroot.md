...
如果此元素存在，则代表布尔值的文本值是必需的。 true **值表示** 文件夹是托管文件夹的根文件夹;false **值表示** 文件夹不是托管文件夹的根文件夹。 
