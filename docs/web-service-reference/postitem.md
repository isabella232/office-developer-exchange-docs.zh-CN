...
**PostItem**元素均表示一个 Exchange 存储中的公告项目。 
...
|[HasAttachments](hasattachments.md) <br/> |表示一项具有至少一个附件如果设置为**true**的属性。 此属性是只读的。  <br/> |
...
|[发件人](from.md) <br/> |代表从中发送公告项的地址。 **从**元素只能在创建时设置。  <br/> |
|[InternetMessageId](internetmessageid.md) <br/> |表示项目的 Internet 消息标识符。  <br/> |
|[IsRead](isread.md) <br/> |指示是否已读取一条消息。  <br/> |
|[PostedTime](postedtime.md) <br/> |表示[PostItem](postitem.md)已发布的时间。  <br/> |
|[引用](references.md) <br/> |代表用于将答复原始消息相关联的新闻标头。  <br/> |
...
|[ReadFlagChange](readflagchange.md) <br/> |读取项目后，在[SyncFolderItems](syncfolderitems.md)响应中返回。 此属性是只读的。 Exchange 2007 SP1 中引入了此元素。  <br/> |
