...
**Message**元素表示 Microsoft Exchange 电子邮件。 
...
|[ReceivedBy](receivedby.md) <br/> |标识代理访问方案中的委派。  <br/> |
|[ReceivedRepresenting](receivedrepresenting.md) <br/> |标识代理访问方案中的主体。  <br/> |
...
|[ReminderMessageData](remindermessagedata.md) <br/> |包含提醒邮件的数据。  <br/> |
...
另一个**message**元素时，可用性操作使用[消息（可用性）](message-availability.md)返回 OOF 邮件。 
  
[Message](message-ex15websvcsotherref.md)元素表示电子邮件以及 Exchange Web 服务（EWS）架构未强类型化的所有其他项目。 项目，如 IPM。共享和 IPM. InfoPath 以**邮件**元素的形式返回。 Exchange 2010 不会在响应中返回基本**项**元素。 
