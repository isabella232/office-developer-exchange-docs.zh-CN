...
**AdjacentMeetings**元素标识所有彼此相邻的会议时间的日历项目。 
...
> [!NOTE]
> 尽管其他子元素都有效每个架构，但的[日历项目](calendaritem.md)元素是 Exchange Web Services (EWS) 将返回**AdjacentMeetings**元素中的唯一子元素。 本主题不会列出的每个架构有效，但不是会通过 EWS 返回子元素。 
