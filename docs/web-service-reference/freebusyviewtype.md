...
|FreeBusyMerged  <br/> |表示包含合并的忙/闲可用性信息流的**FreeBusy**中的所有属性。  <br/> |
|具体  <br/> |代表旧状态信息：闲、忙、暂定和 OOF;约会的开始/结束时间;以及约会的各种属性，如主题、位置和重要性。 此请求的视图将返回请求用户有权限的最大信息量。 如果只提供了合并的忙/闲信息，就像 Microsoft Exchange Server 2003 林中的用户请求信息一样，将返回**MergedOnly** 。 否则，将返回**FreeBusy**或**详细信息**。  <br/> 如果为通讯组列表指定了**详细**信息，则会合并列表成员的忙/闲信息，并返回**MergedOnly** 。  <br/> |
|DetailedMerged  <br/> |表示与合并的忙/闲可用性信息流**有关的所有**属性。 如果仅有合并的忙/闲信息，例如，如果该邮箱存在于运行 Exchange 2003 的计算机上，则将返回**MergedOnly** 。 否则，将返回**FreeBusyMerged**或**DetailedMerged** 。  <br/> |
