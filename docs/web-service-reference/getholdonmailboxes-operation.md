...
**GetHoldOnMailboxes**操作获取正在对特定保留项的邮箱和关联的保留查询。 
  
Exchange Server 2013 中引入了此操作。
  
## <a name="using-the-getholdonmailboxes-operation"></a>使用 GetHoldOnMailboxes 操作

**GetHoldOnMailboxes**操作提供了有关哪些邮箱置于下，特定保留项、 有关与每个保留，如果适用，关联的保留查询的信息和有关每个邮箱的保留状态信息的客户端信息。 有关邮箱保留，包括基于查询的保留项，请参阅 TechNet 上的[就地保留](http://technet.microsoft.com/en-us/library/ff637980%28v=exchg.150%29)。 
...
**GetHoldOnMailboxes**操作请求的下面的示例演示如何获取 HoldId2 邮箱保留的邮箱保留信息。 
...
## <a name="successful-getholdonmailboxes-operation-response"></a>成功 GetHoldOnMailboxes 操作响应

下面的示例演示对**GetHoldOnMailboxes**操作的成功响应请求以获取邮箱保留 HoldId2 邮箱保留的信息。 
...
下面的示例演示对**GetHoldOnMailboxes**操作请求错误响应。 这是对请求以获取保留已删除的响应。 
